...
export default function Rooms({ onBookRoom }) {
  ...
  return (
    <section id="rooms" className="pt-10 pb-6 sm:pt-14 sm:pb-8 relative bg-[#F8FAF8]">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="text-center max-w-3xl mx-auto mb-12 sm:mb-16">
          ...
          >
            <Sparkles className="w-3.5 h-3.5 text-[#C9A227]" /> Opulent Accommodations
          </motion.div>
          <motion.h2
            initial={{ opacity: 0, y: 20 }}
            whileInView={{ opacity: 1, y: 0 }}
          ...
          >
            Designed with rich teakwood, panoramic glass walls, and private backwater balconies. Choose your ideal sanctuary.
          </motion.p>
        </div>
...
function RoomCard({ room, index, onViewDetails, onBookRoom }) {
  ...
  return (
    ...
    >
      <div>
        {/* Image Slider Container: Instagram 4:5 portrait ratio for vertical photos with ZERO black side bars */}
        <div className={`relative ${room.isVertical ? 'w-full aspect-[4/5]' : 'h-64'} overflow-hidden group/slider flex items-center justify-center`}>
          <img
          ...
          {/* Price Tag Pill (Only rendered if price exists) */}
          {room.price && (
            <div className="absolute bottom-4 left-4 px-3.5 py-1.5 rounded-2xl bg-white/95 backdrop-blur-md border border-[#ECECEC] shadow-md flex items-baseline gap-1">
              <span className="font-serif text-lg font-bold text-[#2F6B3E]">{room.price}</span>
              {room.priceUnit && <span className="text-[10px] text-[#555555] font-medium">{room.priceUnit}</span>}
            </div>
          )}
                ...
                {room.images.map((_, idx) => (
                  <span
                    ...
                    className={`w-1.5 h-1.5 rounded-full transition-all ${
                      idx === currentImageIndex ? 'w-4 bg-[#C9A227]' : 'bg-white/60'
                    }`}
                  />
                ))}
              </div>
        ...
        <div className="p-6 space-y-3">
            ...
            <div className="flex items-center justify-between text-xs text-[#555555] font-medium">
              {room.capacity && (
                <span className="flex items-center gap-1 font-bold text-[#2F6B3E]">
                  <Users className="w-3.5 h-3.5 text-[#2F6B3E]" /> {room.capacity}
                </span>
              )}
              {room.size && <span className="text-[#C9A227] font-semibold">{room.size}</span>}
            </div>
          )}
          ...
          {/* Quick Feature Badges - 3rd feature moves cleanly to line 2 below the first two chips */}
